...
$f\colon \mathbb{Z} \to \mathbb{R}$ が凸であることを示すには，$2f(x) \leq f(x-1)+f(x+1)$（または $f(x)-f(x-1) \leq f(x+1)+f(x)$）が言えれば十分です．「$x$ が元々大きいと $1$ 増やすのももっと大変だよね」という議論だけでは厳密には足りていません．よくある証明パターンは，$x-1$ に対する構成と $x+1$ に対する構成を部分的に組み替えて $x$ に対する構成を $2$ つ作ることです．
